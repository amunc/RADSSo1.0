...
'''Mandatory execution parameters - IT IS SUGGESTED NOT TO MODIFY. It can cause impredictable behaviour'''
...
max_lenght_output_names = 180 #in linux system it can be incremented
list_of_parameters_models_events_dict = [glod.get_learning_key(),glod.get_model_path_key(),glod.get_reasignment_dict_key(),glod.get_current_features_key(),glod.get_original_features_key()]
list_names_metrics = [glod.get_precision_key(),glod.get_recall_key(),glod.get_specificity_key(),glod.get_f1_score_key(),glod.get_mcc_key()]
...
log_path = os.path.join(BASE_PATH,auxf.decodify_using_enconding(config_parser.get(logs_section,glod.get_log_directory_name()),char_encoding))
...
try: 
    ...
    errors_file = os.path.join(log_path,auxf.decodify_using_enconding(config_parser.get(logs_section,glod.get_log_error_name()),char_encoding)+target+'.log')
    ...
    print "percentage of features " + str(n_feat)
            
            
    '''Setp 0.2: reading name feature (feature with events names)'''
    name_variable_with_events = auxf.decodify_using_enconding(config_parser.get(input_data_section,glod.get_event_name_feature_name()),char_encoding)
    ...
    errors_file = os.path.join(log_path,auxf.decodify_using_enconding(config_parser.get(logs_section,glod.get_log_error_name()),char_encoding)+target+'.log')    
    repg.register_log([errors_file],"Errors registered:\n",0,char_encoding)           
    log_file = os.path.join(log_path,auxf.decodify_using_enconding(config_parser.get(logs_section,glod.get_log_execution_name()),char_encoding)+target+'.log')    
    time_log = os.path.join(log_path,auxf.decodify_using_enconding(config_parser.get(logs_section,glod.get_log_time_execution_name()),char_encoding)+target+'.log')        
    ...
    path_to_input_files = os.path.join(BASE_PATH,auxf.decodify_using_enconding(config_parser.get(input_data_section,glod.get_path_to_root_directory_input_files_name()),char_encoding))
    path_to_dir_user_files = os.path.join(BASE_PATH,auxf.decodify_using_enconding(config_parser.get(input_data_section,glod.get_user_files_directoryname()),char_encoding))
    auxiliary_directory_filename = auxf.decodify_using_enconding(config_parser.get(auxiliary_data_section,glod.get_auxiliary_directory_parameter_name()),char_encoding)
    ...
    events_filename = auxf.decodify_using_enconding(config_parser.get(input_data_section,glod.get_events_filename_name()),char_encoding)
    ...
    discarded_variables_filename = auxf.decodify_using_enconding(config_parser.get(input_data_section, glod.get_user_discarded_variables_filename_name()),char_encoding)    
    ...
    if(selected_models == True):#Se ha introducido para poder seleccionar n modelos
        ...
        full_list_of_available_models = [customized_supervised_models_dictionary.values(),customized_unsupervised_models_dictionary.values()]
    else:
    ...
    ruta_directorio_raiz_salida = os.path.join(BASE_PATH,auxf.decodify_using_enconding(config_parser.get(output_data_section,glod.get_output_directory_rootname()),char_encoding)) #directorio raiz de salida    
    ...
    path_to_directory_prediction_models = os.path.join(BASE_PATH,auxf.decodify_using_enconding(config_parser.get(output_data_section,glod.get_output_directory_name_prediction_models()),char_encoding))
    ...
    for event_index in range(len(list_of_events)):   
        ...
        path_to_reports_current_event_directory = os.path.join(path_to_current_event_directory,auxf.decodify_using_enconding(config_parser.get(output_data_section,glod.get_output_directory_name_report_name()),char_encoding))            
        ...
        try:            
            ...
            if (not existen_datos_catalogados):                
                ...
                if(solved_problem):#only one target
                    ...
                    if (validation_mode != True):
                        ...
                        #save input data                        MODIFICADO - TO -DO - --NO SE GUARDAN LOS DATOS EN LOS EXPERIMENTOS--
                    ...
                    if(not existence_possible_features_train_models):
                        ...
                        if (feature_selection_method == 'fisher'):
                            print 'Fisher'
                            path_to_fisher_output_directory = os.path.join(path_to_current_event_directory,auxf.decodify_using_enconding(config_parser.get(output_data_section,glod.get_output_directory_name_fisher_name()),char_encoding))
                            ...
                            print 'Mif'                        
                            path_to_mif_output_directory = os.path.join(path_to_current_event_directory,auxf.decodify_using_enconding(config_parser.get(output_data_section,glod.get_output_directory_name_mif_name()),char_encoding))
            ...
            best_model_final_pkl_name = auxf.decodify_using_enconding(config_parser.get(output_data_section,glod.get_output_directory_name_prediction_models()),char_encoding) + '/' + current_event + "_" + target +"_"+best_model[0]+".pkl"
            
            
            pred_path = auxf.decodify_using_enconding(config_parser.get(output_data_section,glod.get_output_directory_name_prediction_models()),char_encoding)
