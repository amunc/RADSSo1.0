...
def metrics_cm(cm):
    ...
    :return list matrix_confusion_tables_by_class: A table for each class with TP,FN,FP,TN distribution
    '''
...
def compute_macro_avg_values_of_metrics(list_names_metrics,precisions_vector, recalls_vector,specificity_vector,f1_score_vector,mcc_vector,target_recodified_values,report_dict,model_name,number_decimals = 4):
    '''
    It allows to compute macro avg values for the vector of values of metrics (precision, recall, specificty, f1-score, mcc) that receives. The means
    ...
    for ind in range(number_targets):
        temp_dict = {glod.generic_target_dict_key() +str(target_recodified_values[ind]):{list_names_metrics[0]:str(precisions_vector[ind]),
...
def save_confusion_matrix(confusion_matrix,customized_labels,customized_name,output_format):
    ...
    plt.figure()
    plt.imshow(confusion_matrix, cmap=plt.cm.Reds, interpolation='nearest')
...
def get_variables_inside_percentil(report,percent,list_important_features_for_the_user):
    '''
        This function allows to obtain the features that follow into the specified percentil of features
        by the user.
...
def apply_mutual_information_function_to_current_features_using_percentil(event,target_variable,percentil_specified_by_user,path_to_directory_current_target,df_catalogued_data,list_relevant_variables_user,useless_features,log_file,enco):
    ...
    repg.create_score_report(path_to_directory_current_target,filename_generated_reports,list_possible_features_mif_scores,percent,enco)
...
def apply_fisher_score_to_current_features_using_percentil(event,target_variable,percentil_specified_by_user,path_to_directory_current_target,df_catalogued_data,list_relevant_variables_user,useless_features,log_file,enco):
    ...
    '''Generating file with features and scores'''
    repg.create_score_report(path_to_directory_current_target,filename_generated_reports,list_possible_features_fisher_scores,percent,enco)
