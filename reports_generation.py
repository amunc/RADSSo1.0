...
def update_report_full_list_features_used_in_process(report_dict,lista_variables_in_process):
    report_dict[glod.get_report_general_info_key()][glod.get_variables_key()][glod.get_used_in_process()] = lista_variables_in_process
    return report_dict
...
def update_report_current_model_decision_index(report_dict,modelo,valor): #'Decision index'
...
def create_score_report(directorio_salida,nombre_informe,report,percentil,enco):
    try:
...
#la funcion guardar_datos_en_fichero permite almacenar diferentes estructuras de datos en ficheros con diferentes extensiones
#recibe los parametros: datos (pandas dataframe o numpy_array), ruta_directorio (directorio de almacenamiento del informe),
#nombre_fichero (nombre del informe donde se guardaran los datos), delimitador (separador de campos) extension (extension del fichero)
