...
def select_optimal_parameters_current_model(model_name,diccionario_modelos_sup,diccionario_modelos_no_sup,df_parameter_selection,features,target,fichero_log,ruta_directorio_modelo_supervisado,report_dict,formato,enco,max_pars,exponent):
    ...
    if(model_name == diccionario_modelos_sup[1]):#Tree
        ...
        try:
            ...
            parameters, accuracy, ac, num_evals= rhoaso.selection_n(GradientBoostingClassifier, df_parameter_selection[features], df_parameter_selection[target],max_pars_ensemble,1,exponent=exponent)# num_neurons=None, random=False,n_layers=None)            
        ...
        try:
            name='accuracy_matrix_'+model_name
            parameters,accuracy, ac, num_evals=rhoaso.selection_n(MLPClassifier, df_parameter_selection[features], df_parameter_selection[target],max_pars_mlp,1,exponent=exponent)#, num_neurons=None, random=False,n_layers=None)
